...
export class RegisterComponent {
    ...
    register() {
        ...
        if (this.ngForm.form.invalid) {
            ...
            this.checkPictureUrl(this.ngForm.form.controls.pictureUrl);
            return;
        }
        ...
        this.loading = true;
        if(this.checkPictureUrl(this.model.pictureUrl)) {
    ...
    checkPictureUrl(url) : boolean {
        if(url != undefined &&  !(/(https?:\/\/.*\.(?:gif|jpg|jpeg|tiff|png))/i).test(url)) {
            this.errors.validPictureUrl = true;
            return false;    
        } else return true;
    }
}
